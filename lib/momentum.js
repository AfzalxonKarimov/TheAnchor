...
// Momentum gained per completed session (base).
const MOMENTUM_PER_SESSION = 2;

// Sustained streak bonus, applied per session by tier (NOT one-time). You earn
// the single highest tier you currently qualify for — so on a 20-day streak you
// keep the +15 (14+) tier every session, and tiers don't stack (day 30 is +25,
// not 5+10+15+25). Ordered high → low so the first match wins.
...
/**
 * Highest streak-tier bonus the given streak qualifies for (0 below the first tier).
 * @param {number} streak
 * @returns {number}
 */
export function getStreakBonus(streak) {
  const s = streak || 0;
  const tier = STREAK_BONUS_TIERS.find((t) => s >= t.minStreak);
 ...
 */
export function calculateMomentumChange({ duration, currentMomentum, streak }) {
  let change = MOMENTUM_PER_SESSION;

  // Bonus for session duration (longer = more momentum)
  const minutes = Math.floor((duration || 0) / 60);
  if (minutes >= 15) change += 1;
  if (minutes >= 30) change += 1;

  // Sustained streak bonus: the highest tier the current streak qualifies for.
  change += getStreakBonus(streak);
 ...
 */
export async function updateMomentum({ userId, durationSeconds, streak }) {
  const { supabase } = await import('../src/supabase/client');
